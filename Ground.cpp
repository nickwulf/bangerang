...
bool Ground::testInsideLine(float point[], float vert1[], float vert2[]) { // this tests if a point is on the same side of the line as the center
	...
	if ((vert2[0]-vert1[0]) == 0) {  // if slope = infinite
		if (center[0] > vert1[0]) testCenter = true;
		if (point[0] > vert1[0]) testPoint = true;
	}
	else {
		...
		if (center[2] > slope*center[0] + yIntercept) testCenter = true;
		if (point[2] > slope*point[0] + yIntercept) testPoint = true;
	}
	if (testPoint == testCenter) testInside = true;
	return testInside;
}
